...
app.post("/start-qr", (req, res) => {
  ...
  // Increment total attendance for all students
  const sql = `UPDATE students_attendance SET ${subject}_total = ${subject}_total + 1`;
  db.query(sql, err => {
    if (err) console.error(err);
  });

  res.json({ subject, token, message: "QR started and total incremented" });
});
...
app.post("/stop-qr", (req, res) => {
  ...
  if (!activeQRSessions[subject])
    return res.status(400).json({ message: "QR not running" });
